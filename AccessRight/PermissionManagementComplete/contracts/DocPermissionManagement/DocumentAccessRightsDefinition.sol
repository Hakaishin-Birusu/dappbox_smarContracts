...
contract DocumentAccessRightsDefinition is DocNotarizationManagement{
  ...
  */
  ...
  event NewDocument(bytes32 _hash, address master, uint blockNumber);
  ...
  */
  function checkPermission(address _delegate, bytes32 _folderAddress) internal view returns (PermissionType){
  ...
  * @param _folderAddress folderAddress of the NewFolder
  */
  function checkTimedPermission(address _delegate, bytes32 _folderAddress) private view returns(PermissionType){
  ...
  * @param _folderAddress folderAddress of the NewFolder
  */
  function checkPermanentPermission(address _delegate, bytes32 _folderAddress) internal view returns(PermissionType){
  ...
  * @param _folderAddress folderAddress of the NewFolder
  */
  function isOwner(address _delegate, bytes32 _folderAddress) private view returns (bool){
  ...
  * @param _folderAddress folderAddress of the NewFolder
  */
  function isWriter(address _delegate, bytes32 _folderAddress) private view returns (bool){
  ...
  * @param _folderAddress folderAddress of the NewFolder
  */
  function isReader(address _delegate, bytes32 _folderAddress) private view returns (bool){
  ...
  */
  function upgradeToOwner(address dAppBoxOrigin,address _delegate, bytes32 _folderAddress, uint256 _fromTime, uint256 _toTime) public returns (bool success){
    require(msg.sender == dAppBoxOrigin,"authentication failed");
    require(documentExists(_folderAddress), "Folder does not exists");
